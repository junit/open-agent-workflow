...
func validatePreToolUseInput(input PreToolUseInput) error {
	if input.HookEventName != "PreToolUse" || !validHookText(input.SessionID, 512) ||
		...
		!validHookText(input.PermissionMode, 128) || input.ToolName != "mcp__oaw_codex_bridge__observe_profile" {
		return codexbridge.NewError("HOST_BRIDGE_CONTEXT_REQUIRED", "Hook input identity or tool is invalid", nil)
	}
...
func bridgeToolOperation(value string) (codexbridge.Operation, bool) {
	if value != "mcp__oaw_codex_bridge__observe_profile" {
		return "", false
	}
	operation, err := codexbridge.ParseOperation(string(codexbridge.OperationObserveProfile))
