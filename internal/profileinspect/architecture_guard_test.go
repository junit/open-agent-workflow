...
func TestProfileInspectionHasNoMachineAuthorityDependency(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate profileinspect package")
	}
		...
		for _, imported := range parsed.Imports {
			...
			for _, fragment := range forbidden {
				if strings.Contains(path, fragment) {
					t.Errorf("%s imports machine authority package %q", entry.Name(), path)
				}
			}
		}
	}
