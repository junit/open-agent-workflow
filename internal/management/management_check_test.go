...
func TestCheckManagementDomainExercisesDiagnosticsAndHealth(t *testing.T) {
	t.Run("empty user", func(t *testing.T) {
		...
		result, err := Check(fixture.environment, CheckRequest{})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("detected project targets", func(t *testing.T) {
		fixture := newPrepareFixture(t)
		...
		result, err := Check(fixture.environment, CheckRequest{Project: project})
		if err != nil {
			t.Fatal(err)
		}
		for _, expected := range []string{
	...
	t.Run("clean project owned file", func(t *testing.T) {
		...
		materializePreparedFixture(t, prepared)
		result, err := Check(fixture.environment, CheckRequest{Project: project, Targets: "cursor"})
		...
		writePrepareFile(t, prepared.targetActions[0].destination, prepared.targetActions[0].data, 0o644)
		result, err = Check(fixture.environment, CheckRequest{Project: project, Targets: "roo"})
...
func TestProjectCheckDetectsPolicySetHealthWithoutOwningCustomProfiles(t *testing.T) {
	...
	for name, mutate := range mutations {
		t.Run(name, func(t *testing.T) {
			...
			result, err := Check(fixture.environment, CheckRequest{Project: project, Targets: "codex"})
...
func TestUserCheckDetectsPolicySetHealthWithoutOwningCustomProfiles(t *testing.T) {
	mutations := map[string]func(*testing.T, string){
		"missing": func(t *testing.T, path string) {
			if err := os.Remove(path); err != nil {
				t.Fatal(err)
		...
		"unexpected managed file": func(t *testing.T, path string) {
			writePrepareFile(t, filepath.Join(filepath.Dir(path), "injected.md"), []byte("injected\n"), 0o644)
		},
	}
	for name, mutate := range mutations {
		t.Run(name, func(t *testing.T) {
			...
			customProfile := filepath.Join(fixture.environment.ConfigHome, "open-agent-workflow", "profiles", "changed.md")
			writePrepareFile(t, customProfile, []byte("user-owned\n"), 0o644)
			result, err := Check(fixture.environment, CheckRequest{Targets: "codex"})
			if err != nil || !resultContainsLine(result, "installed codex: clean") {
				t.Fatalf("clean Check() result=%#v error=%v", result, err)
			}

			mutate(t, filepath.Join(fixture.environment.ConfigHome, "open-agent-workflow", "adapters", "codex-policy.md"))
			result, err = Check(fixture.environment, CheckRequest{Targets: "codex"})
			if err != nil || !resultContainsLine(result, "installed codex: drift") {
				t.Fatalf("drift Check() result=%#v error=%v", result, err)
			}
...
func TestCheckManagementDomainRejectsInvalidRequestsAndPreservesPartialResult(t *testing.T) {
	...
	for name, request := range map[string]CheckRequest{
		...
		t.Run(name, func(t *testing.T) {
			if _, err := Check(fixture.environment, request); err == nil {
