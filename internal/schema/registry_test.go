...
func TestRegistryValidatesWorkflowCoordinatorSchemaFamily(t *testing.T) {
	...
	digest := strings.Repeat("a", 64)
	command := []byte(`{"schema_version":"oaw.workflow-command/v2","kind":"INSPECT","message_id":"","idempotency_key":"","workflow_id":"workflow-0123456789abcdef0123456789abcdef","expected_revision":0}`)
	if err := registry.Validate(WorkflowCommandV2, command); err != nil {
		t.Fatalf("Validate(WorkflowCommandV2) error = %v", err)
	}
	invalidInspect := []byte(strings.Replace(string(command), `"workflow_id":"workflow-0123456789abcdef0123456789abcdef"`, `"workflow_id":""`, 1))
	if err := registry.Validate(WorkflowCommandV2, invalidInspect); err == nil || !strings.Contains(err.Error(), "SCHEMA_VALIDATION_FAILED") {
		t.Fatalf("Validate(empty INSPECT workflow_id) error = %v", err)
	}
	mixed := []byte(`{"schema_version":"oaw.workflow-command/v2","kind":"INSPECT","message_id":"","idempotency_key":"","workflow_id":"workflow-0123456789abcdef0123456789abcdef","expected_revision":0,"cancel":{"reason":"stop","invocation_terminal":true}}`)
	if err := registry.Validate(WorkflowCommandV2, mixed); err == nil || !strings.Contains(err.Error(), "SCHEMA_VALIDATION_FAILED") {
	...
	for _, retired := range []string{
		...
		if err := registry.Validate(retired, []byte(`{}`)); err == nil || !strings.Contains(err.Error(), "UNKNOWN_SCHEMA") {
			t.Fatalf("retired Coordinator schema %s error = %v", retired, err)
		}
	}
