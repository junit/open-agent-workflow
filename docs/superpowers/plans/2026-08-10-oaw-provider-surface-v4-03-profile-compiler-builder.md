...
- A macro parent or child appears once at `AnchorSlotID`, the earliest slot in its ordered contiguous `SlotIDs` for which that Binding declares `OutcomeOwner=true`; a Binding without an outcome claim falls back to the first slot in its span. This separates a macro envelope from execution order, so a workspace child can anchor before an implementation parent. The unit's `SlotIDs` records every enveloped slot. A credited slot's `OutcomeOwner` references that Unit ID only when the Recipe designates it as the slot outcome owner; no credited slot duplicates the pipeline unit.
- Mandatory suppression, cycles, peer duplication, duplicate credit/dispatch, undeclared alternatives, and conflicting responsibility namespaces return `MACRO_INTERNAL_CONFLICT`. SDD has no declared `test-driven-development` InternalCall, so a USER-DEFINED Matt-SDD composition may retain Matt as the single TDD owner when Superpowers TDD is paused.
- `grill-with-docs` credits `grilling` and `domain-modeling` once. SDD dispatches workspace before and finish after; its prompt-driven per-task/final review is an embedded SDD responsibility, while TDD and fresh verification remain standalone Recipe units. Inline `executing-plans` dispatches workspace before and finish after while retaining standalone TDD/review/verification.
