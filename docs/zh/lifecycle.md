...
Skill 不可用时，使用 Profile 或 Policy 明确的替代方案，或说明限制并询问用户。不要臆造 Skill 可用性、
结果或 recovery owner。
...
`oaw-assurance` 可以证明精确 Profile 内容和 Skill Binding 身份；`oaw-bridge` 可以向该证据路径提供
当前 Codex Host observation。它们不管理交付，缺失或失败也不能阻断正常 Policy 生命周期。
