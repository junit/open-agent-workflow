...
Bridge v1 只支持 `CURRENT` topology。当前 Codex session 是 physical executor。没有 `SUBAGENT` implementation、shell fallback、container fallback、隔离用户主目录、projected configuration，也没有复制 MCP、Hook、Skill、Plugin、model、authentication、sandbox 或 approval environment 的行为。除非 Codex 报告稳定且 allowlisted 的 evidence，否则 Host surface 保持 `unknown`。
...
`core.inspect` 解析 Host-scoped Provider Candidate 与 exact enabled Skill binding。v1 以 `skills/list` 作为 Provider binding authority。只有 exact name 和 normalized path 同时映射到恰好一个 discovered Provider installation 与 Capability 时 Skill 才能 verify。Disabled、missing、orphan、ambiguous 或 foreign-Host Skill 都不会创建 authority。Superpowers、Matt、ECC 与 user Provider 使用相同 matching algorithm；没有按 brand 放行 Candidate 的例外。

`core.compile` 接受用户显式选择的 Profile 与 `CURRENT`。`WORKFLOW` 仍必须经过 Startup Gate；Bridge observation 只提供 Host evidence，不选择 Profile。`workflow_exchange` 交换编译后的 Bundle 与 Coordinator record。Codex 仍负责执行选定的 Skill 与 tool。
...
Host integration diagnostic 与 Provider、Profile decision 分离。以下是稳定的 v1 reason。每项 recovery 都是显式动作，不会静默选择 Profile 或改变 Host authority。
...
| `HOST_EVIDENCE_EXPIRED` | Handle 超过 bounded TTL。 | 再次调用 `observe_current`，然后执行 `core.inspect` 或 `core.compile`。 |
...
| `HOST_OBSERVATION_PARTIAL` | 可选 environment metadata 不完整。 | 再次运行 `core.inspect`，所有不可用 field 保持显式 `unknown`。 |
...
Hook 是唯一的 current-session identity source。Skill instruction、prompt、filesystem candidate 或 user-authored input 都不能替代它。`skills/list` 是 v1 唯一的 Provider binding authority。`plugin/list` 不是 production dependency。Raw Hook command、credential、MCP environment value、header、token、arbitrary Plugin setting 与 App Server configuration 都不会被保存。
...
不要把 handle、absolute Skill path、raw Hook command 或 credential 发布到 ticket、Workflow State、evidence、log 或 screenshot。Local dogfood record 放在 `.scratch/oaw-codex-host-bridge-dogfood/` 且保持 untracked。
