...
不读取完整 Policy。下表中，**documented import** 指 provider 定义的文件导入能力；OAW 不用它来
急切导入 Policy。
...
| Cline | Cline 的主要 project rule 格式是 `.clinerules/`，会处理其中的 `.md` 与 `.txt` 文件。它还检测包括 `AGENTS.md` 在内的多个兼容文件。发生冲突时，workspace rule 优先于 global rule。 | OAW 创建一个含 Activation Router 的 owned `.clinerules/open-agent-workflow.md` 文件。 | Cline 支持带 YAML `paths` frontmatter 的 conditional rule；没有 frontmatter 表示 Router 始终可见。它仍是惰性 Router：普通工程工作在显式激活前保持原生 Host 行为。 | <https://docs.cline.bot/customization/cline-rules> |
...
`AGENTS.md` 视为 shared managed-block destination，渲染一个连贯的 Activation Router，而不是
两个独立 block。这样既保留无关 project instruction，也避免重复 Router text。
...
- Claude 和 Gemini 不会输出 `@` 来导入 canonical policy。它们的 Router 只在显式激活后读取该 Policy。
- Codex 与 OpenCode 的 instruction Markdown file 没有文档化的自动 Markdown import；
  四个 target 都使用同一个 model-visible Activation Router 语义。
