...
- [x] Host manifests are pinned through trusted integration records and cannot
  self-enable features from per-run frames.
- [x] Instruction-only integrations cannot claim Runtime admission or isolation.
- [x] Runner-managed and native-managed integrations pass fixtures for isolated
  Executor creation, exact Binding invocation, pause behavior, Bundle
  inheritance, evidence return, and invocation deduplication.
- [x] Missing required features deny Runtime-managed Workflow with a stable reason
  code.
- [x] The first Runtime Host remains unselected until the official capability audit
