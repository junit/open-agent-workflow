# MATT-SP-HYBRID Dogfood Specification
...
`HH:MM-HH:MM` windows. It accepts Boundary Touch, rejects Overlap, and reports
malformed or backwards windows without writing state.
...
5. As a release operator, I want input order to be irrelevant to conflict
   detection, so that plans can be validated as written by different tools.
