...
digest_header() {
  ...
  if command -v shasum >/dev/null 2>&1; then
    printf 'oaw.host-evidence-handle/v1\000%s\000%s' "$kind" "$value" |
...
{"version":"oaw.host-evidence-handle/v1","session_digest":"$session_digest","cwd_digest":"$cwd_digest","token":"opaque-test-token"}
EOF
...
(cd "$OAW_REPOSITORY" && go test ./internal/codexbridge -run 'TestBuildBindingInventoryRejectsDisabledOrphanAndUnboundSkills|TestCandidatePathRejectsForeignHost|TestEvidenceStoreExpiresAndEvictsDeterministically') >/dev/null
