...
if [ "$mode" != "--write" ] && [ "$mode" != "--check" ]; then
  printf 'usage: %s --write|--check MANIFEST [--matt-root DIR --superpowers-root DIR --ecc-root DIR]\n' "$0" >&2
  exit 64
fi
if [ -z "$target" ]; then
...
if [ -n "$matt_root$superpowers_root$ecc_root" ]; then
  if [ -z "$matt_root" ] || [ -z "$superpowers_root" ] || [ -z "$ecc_root" ]; then
    printf 'all three Provider roots are required\n' >&2
    exit 64
  fi
else
...
if [ "$mode" = "--write" ]; then
  (cd "$repository" && go run ./cmd/oaw-provider-audit --write --output "$target" --matt-root "$matt_root" --superpowers-root "$superpowers_root" --ecc-root "$ecc_root")
else
  (cd "$repository" && go run ./cmd/oaw-provider-audit --check --manifest "$target" --matt-root "$matt_root" --superpowers-root "$superpowers_root" --ecc-root "$ecc_root")
fi
